...
2. **Cleaning** (`src/data/make_dataset.py`) → `data/processed/train_clean.parquet`
3. **Feature Engineering** (`src/features/build_features.py`) → `data/processed/train_features.parquet`
4. **Time-Series CV** (`src/evaluation/cv.py`) → expanding window splits
5. **Model Training** (`src/models/`) → per-fold predictions
6. **Ensemble** (`src/models/ensembles.py`) → weighted blend or stacking
7. **Final Model** → predictions saved to `outputs/predictions/`
...
2. **Forward-looking features** - causes leakage and overfitting
3. **Ignoring `Sales=0` in RMSPE** - Kaggle ignores these observations
4. **Not grouping by Store** - lag/rolling features must be per-store
5. **Random CV splits** - must use time-based expanding window
6. **Training on closed stores** - filter `Open=0` or handle separately
