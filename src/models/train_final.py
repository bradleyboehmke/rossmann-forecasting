...
Trains the final model on the full historical dataset and generates predictions for a 6-week holdout
period (simulating the test set).
"""
...
def create_holdout_split(
    df: pd.DataFrame, holdout_days: int = 42
...
def train_final_ensemble(
    ...
    cat_features = [col for col in valid_features if train_df[col].dtype in ["object", "category"]]
    ...
    xgb_model = xgb.train(
        xgb_params, dtrain, num_boost_round=1600, evals=[(dtrain, "train")], verbose_eval=False
    )
    ...
    ensemble = {
        "models": {"lightgbm": lgb_model, "xgboost": xgb_model, "catboost": cb_model},
        "weights": weights,
...
def predict_with_ensemble(ensemble: dict[str, Any], test_df: pd.DataFrame) -> np.ndarray:
    ...
    lgb_preds = ensemble["models"]["lightgbm"].predict(
        X_test_lgb, num_iteration=ensemble["models"]["lightgbm"].best_iteration
    )
...
def evaluate_final_model(
    predictions: np.ndarray, actuals: np.ndarray, holdout_df: pd.DataFrame
) -> dict[str, Any]:
...
def save_final_model(
    ensemble: dict[str, Any], metrics: dict[str, Any], output_dir: str = "models/final"
):
